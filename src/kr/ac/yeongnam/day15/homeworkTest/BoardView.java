package kr.ac.yeongnam.day15.homeworkTest;
...
public class BoardView {
		...
		try {
			...
			if(num == 0) {
				...
				for(int cnt = 1; cnt <= num; cnt++) {
					int nameTmp = boardArr[cnt-1].getNum();
					String titleTmp = boardArr[cnt-1].getTitle();
					String writerTmp = boardArr[cnt-1].getWriter();
					String dateTmp = boardArr[cnt-1].getDate();
	...
	public void printNum() {
		try {
			...
			System.out.printf("%s\t %-20s\t %s\t %s\n", "번호", "제목", "글쓴이", "등록일");
			System.out.printf("%d\t %-20s %s\t %s\n", 
					boardArr[numTmp-1].getNum(), boardArr[numTmp-1].getTitle(), 
					boardArr[numTmp-1].getWriter(), boardArr[numTmp-1].getDate());
